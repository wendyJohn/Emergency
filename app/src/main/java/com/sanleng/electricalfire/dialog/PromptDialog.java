...
public class PromptDialog {
	...
	/**
	 * 设置进入 进出动画持续的事件默认100毫秒
	 *
	 * @param viewAnimDuration
	 *            毫秒
	 */
	...
	private void initAnim(int widthPixels, int heightPixels) {
		...
		ScaleAnimation scaleAnimation = new ScaleAnimation(2, 1f, 2, 1f, widthPixels * 0.5f, heightPixels * 0.45f);
		...
		scaleAnimation = new ScaleAnimation(1, 2, 1, 2, widthPixels * 0.5f, heightPixels * 0.45f);
		...
		scaleAnimation = new ScaleAnimation(1, 1, 1, 1, widthPixels * 0.5f, heightPixels * 0.5f);
	...
	public ImageView showAd(boolean withAnim, AdClickListener listener) {
	...
	public void showLoading(String msg, boolean withAnim) {
		...
		if (promptView.getCurrentType() != PromptView.PROMPT_LOADING) {
			Builder builder = Builder.getDefaultBuilder();
			builder.icon(R.drawable.loading2);
			builder.text(msg);
			promptView.setBuilder(builder);
			closeInput();
			checkLoadView(withAnim);
