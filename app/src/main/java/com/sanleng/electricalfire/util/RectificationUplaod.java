...
import com.sanleng.electricalfire.util.CustomMultipartEntity.ProgressListener;
import java.io.File;
...
public class RectificationUplaod extends AsyncTask<String, Integer, String> {
    ...
    public RectificationUplaod(Context context, List<String> filePathList, String ids, String pointId, String desc, String username,
                               Handler handler) {
        ...
        try {
            ...
            if (serverResponse != null) {
                if (mymsg.equals("电气火灾隐患处理成功！")) {
                    ...
                    new SVProgressHUD(context).showErrorWithStatus("上传失败" + mymsg);
                }
            } else {
                new SVProgressHUD(context).showErrorWithStatus("上传失败" + mymsg);
            }
