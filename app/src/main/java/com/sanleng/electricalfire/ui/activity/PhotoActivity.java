...
public class PhotoActivity extends BaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < Bimp.drr.size(); i++) {
			...
			imageDescriptionArray = drr.get(i).replaceAll("\\\\", "/").split("/");
			String fileName = "";
			if (imageDescriptionArray.length > 1) {
				fileName = imageDescriptionArray[imageDescriptionArray.length - 1];
				list.add(fileName);
			}
		}
		max = Bimp.max;
		// ���ù�����͵ײ����㡱ѡ��״̬
		tvDescription.setText(list.get(0));
	...
	private OnPageChangeListener pageChangeListener = new OnPageChangeListener() {
		...
		public void onPageSelected(int arg0) {// 页面选择响应函数
			count = arg0;
			// ���ù�����
			tvDescription.setText(list.get(arg0));
